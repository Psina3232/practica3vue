    ...
    methods: {
      addCard(columnIndex) {
        if (columnIndex === 0 && this.isFormValid) {
          ...
          this.sortCardsByPriority(columnIndex); // Сортировка по приоритету
          this.clearNewCard();
        }
      },
      ...
      moveToInProgress(originalCard, columnIndex, cardIndex) {
        ...
        this.columns[columnIndex].cards.splice(cardIndex, 1);
        this.sortCardsByPriority(inProgressIndex); // Сортировка при переходе
      },
      moveToTesting(originalCard, columnIndex, cardIndex) {
        ...
        this.columns[columnIndex].cards.splice(cardIndex, 1);
        this.sortCardsByPriority(testingIndex); // Сортировка при переходе
      },
      moveToDone(originalCard, columnIndex, cardIndex) {
        ...
        this.columns[columnIndex].cards.splice(cardIndex, 1);
        this.sortCardsByPriority(doneIndex); // Сортировка при переходе
      },
      returnToInProgress(card, columnIndex, cardIndex) {
        ...
        this.columns[columnIndex].cards.splice(cardIndex, 1);
        this.sortCardsByPriority(inProgressIndex); // Сортировка при возврате
      }
    }
  });